 ...
 * @param  {Object} classes The optional map of class names to transformers.
 * @return {Object}         The deserialized record.
 */
function deserialize(inputBuffer, classes) {


  if (!inputBuffer) {
    return null;
  }
  var input = inputBuffer.toString('utf8');
  var record = {'@type': 'd'},
    chunk, key, value;
 ...
 */
function eatValue(input, classes) {
  var c, n;
  c = input.charAt(0);
 ...
 */
function eatArray(input, classes) {
  var length = input.length,
    array = [],
    chunk, c;
 ...
 */
function eatRecord(input, classes) {
  var record = {'@type': 'd'},
    chunk, c, key, value;
...
function eatBag(input) {
  ...
  if (baseDeserializer.enableRIDBags) {
