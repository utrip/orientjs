...
Operation.prototype.consume = function (buffer, offset) {
  ...
  if (this.status === Operation.PENDING) {
	...
	if(liveMessage === 114 ){
  	  ...
  	  var recordBuffer = new Buffer(contentLenght);
	  buffer.copy(recordBuffer,0,offset, offset + contentLenght);
...
Operation.prototype.parseRaw = function (buffer, offset, context, fieldName, reader) {
  var length = buffer.readInt32BE(offset);
  offset += constants.BYTES_INT;
  if (length < 0) {
    context[fieldName] = null;
    ...
    console.log(value);
    context[fieldName] = csv.eatValue(value);
  }
  if (reader) {
    reader.call(this, context, fieldName);
  }
...
Operation.prototype.parsePushedData = function (buffer, offset, context, fieldName, reader) {
  var length = buffer.readInt32BE(offset),asString;
  offset += constants.BYTES_INT;
  var recordBuffer = new Buffer(length);
  buffer.copy(recordBuffer,0,offset+1, offset+1 + length);
  asString = buffer.toString('utf8',offset,offset+1);
  switch (asString.charAt(0)) {
    case 'R':
      context[fieldName] = deserializer.deserialize(recordBuffer, this.data.transformerFunctions);
      break;
    default:
      console.log('unsupported pushed data format: ' + recordBuffer.toString('utf8'));
  }
  if (reader) {
    reader.call(this, context, fieldName);
  }
